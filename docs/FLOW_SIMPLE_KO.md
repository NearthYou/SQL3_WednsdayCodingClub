...
```mermaid
flowchart TD
    A["src/cli/main.c<br/>SQL 파일 열기"] --> B["문장 분리<br/>주석 제거, ; 기준 분리"]
    B --> C["src/legacy/lexer.c<br/>토큰화"]
    C --> D["src/legacy/parser.c<br/>Statement 생성"]
    D --> E["src/legacy/executor.c<br/>get_table로 CSV 캐시 로드"]
    E --> F{"문장 종류"}
