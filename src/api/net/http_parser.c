...
int parse_http_request(int fd, ParsedHttpRequest *request, int *status_code, char **error_message) {
    char header_buf[MAX_HEADER_BYTES + 1];
    ssize_t nread;
    size_t total = 0;
    char *header_end;
    size_t header_len;
    ...
    char *line;
    char *saveptr = NULL;
    size_t copied_from_first_read = 0;

    if (!request) return set_error(status_code, error_message, 500, "internal parser error");
    memset(request, 0, sizeof(*request));
    if (status_code) *status_code = 0;
    if (error_message) *error_message = NULL;

    while (total < MAX_HEADER_BYTES) {
        nread = recv(fd, header_buf + total, MAX_HEADER_BYTES - total, 0);
        if (nread == 0) return set_error(status_code, error_message, 400, "client closed connection");
    ...
    header_end = strstr(header_buf, "\r\n\r\n");
    if (!header_end) return set_error(status_code, error_message, 400, "request header too large");
    header_len = (size_t)(header_end - header_buf) + 4;
    ...
    while ((line = strtok_r(NULL, "\r\n", &saveptr)) != NULL) {
        ...
        *colon = '\0';
        ...
        if (starts_with_ci(line, "Content-Length")) {
            ...
            if (!endptr || *endptr != '\0') return set_error(status_code, error_message, 400, "invalid Content-Length");
    ...
    if (total > header_len) {
        copied_from_first_read = total - header_len;
        if (copied_from_first_read > request->content_length) {
            free_http_request(request);
            return set_error(status_code, error_message, 400, "request body length mismatch");
        }
        memcpy(request->body, header_buf + header_len, copied_from_first_read);
        body_read = copied_from_first_read;
    }

    while (body_read < request->content_length) {
        nread = recv(fd, request->body + body_read, request->content_length - body_read, 0);
        if (nread <= 0) {
            free_http_request(request);
            return set_error(status_code, error_message, 400, "failed to read request body");
        }
        body_read += (size_t)nread;
    }
